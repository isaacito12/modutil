classdef TimedTraceBuilderAppMain < handle
  ...
  properties
    ...
    Window AppUtil1.AppUtilWindow

    RandomSeedUI AppUtil1.Component.PhysicalValueUI

    DataUnitUI AppUtil1.Component.DropDown

    InitialDataValueUI AppUtil1.Component.PhysicalValueUI
    InitialConstantDurationUI AppUtil1.Component.PhysicalValueUI

    InitialTransitionDurationUI AppUtil1.Component.PhysicalValueUI

    NumberOfTransitionsUI AppUtil1.Component.PhysicalValueUI
    RangeOfTransitionDurationUI AppUtil1.Component.PhysicalValueUI
    RangeOfConstantDurationUI AppUtil1.Component.PhysicalValueUI
    RangeOfDataValueUI AppUtil1.Component.PhysicalValueUI

    FinalTransitionDurationUI AppUtil1.Component.PhysicalValueUI

    FinalDataValueUI AppUtil1.Component.PhysicalValueUI
    FinalConstantDurationUI AppUtil1.Component.PhysicalValueUI
  ...
  properties (Constant, Access=private)
    ...
    unit_ui_width = AppUtil1.Constant.Width{"unitwidth"} * 8
  ...
  methods
    ...
    function App = TimedTraceBuilderAppMain(NameValuePair)
      ...
      App.Window = AppUtil1.AppUtilWindow;
...
% !todo: delete thse settings once getParam is available.
    ...
    function build_app_gui(App)
      ...
      link_ui = AppUtil1.Component.Hyperlink(NewSlot(layout, row));
      link_ui.Text = "Description";
      link_ui.HyperlinkClickedCallback =  @() web(html_file);

      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      % label_ui = AppUtil1.Component.Label(NewSlot(layout, row, Width="fit"));
      label_ui = AppUtil1.Component.Label(NewSlot(layout, row));
      label_ui.Text = "\textbf{Parameters}";
      % label_ui.ComponentWidth = App.width_unit * 15;

      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.RandomSeedUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      App.DataUnitUI = AppUtil1.Component.DropDown(NewSlot(layout, row));
      App.DataUnitUI.Items = App.data_value_units;
      App.DataUnitUI.ValueChangedCallback = @() auto_update_plot(App);

      space_ui = AppUtil1.Component.Label(NewSlot(layout, row, Width="fit"));
      space_ui.Text = "";
      space_ui.ComponentWidth = App.unit_ui_width;

      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.InitialDataValueUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.InitialConstantDurationUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.InitialTransitionDurationUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.NumberOfTransitionsUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.RangeOfTransitionDurationUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.RangeOfConstantDurationUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.RangeOfDataValueUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.FinalTransitionDurationUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.FinalDataValueUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.FinalConstantDurationUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.TableGridVectorUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.TableValuesUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      App.PlotButtonUI = AppUtil1.Component.EnabledButton(NewSlot(layout, row, Width="fit"));
      ...
      App.OpenInFigureWindowUI = AppUtil1.Component.Hyperlink(NewSlot(layout, row));
      ...
      App.AxesUI = AppUtil1.Graphics.Axes(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      App.IntervalUI = AppUtil1.Component.PhysicalValueUI(NewSlot(layout, row));
      ...
      % -----------------------------------------------------------------------
      row = NewRow(layout, column);
      AppUtil1.Component.HorizontalLine(NewSlot(layout, row));

      % -----------------------------------------------------------------------
      % Configure the block selector UI to find Simscape PS Lookup Table (1D) block and
      % Simulink 1-D Lookup Table block.
      row = NewRow(layout, column);
      App.SelectorUI = AppUtil1.Component.BlockSelectorUI(NewSlot(layout, row));
    ...
    function update_plot(App, NameValuePair)
      ...
      App.TableGridVectorUI.Value = CodeUtil1.stringify(t_data');
      ...
      App.TableValuesUI.Value = CodeUtil1.stringify(f_data');
    ...
    function data_table = get_signal_design_matrix_from_UI_components(App)
      ...
      result_random_seed = CodeUtil1.getNumericValueFromString(App.RandomSeedUI.Value);
      ...
      result_num_trans = CodeUtil1.getNumericValueFromString(App.NumberOfTransitionsUI.Value);
      num_trans = result_num_trans.NumericValue;

      trans_du_range = App.RangeOfTransitionDurationUI.Value;
      range_of_transition_duration = CodeUtil1.getNumberArrayFromString(trans_du_range);

      const_dur_range = App.RangeOfConstantDurationUI.Value;
      range_of_constant_duration = CodeUtil1.getNumberArrayFromString(const_dur_range);

      dat_val_range = App.RangeOfDataValueUI.Value;
      range_of_data_value = CodeUtil1.getNumberArrayFromString(dat_val_range);
