...
if SubsystemPath == ""
  if isempty(gcs)
    ...
    msg = CodeUtil1.i18n("Model must be specified.");
...
all_blocks = ModelUtil1.findLookupTable1DBlocks(SubsystemPath, SearchDepth = NameValuePair.SearchDepth);
num_blocks = numel(all_blocks);
if num_blocks == 0

  id = errorID + "BlocksWereNotFound";
  msg = CodeUtil1.i18n("No lookup table blocks were found.");
...
if not(isfield(NameValuePair, "Blocks"))
  ...
  for idx = 1 : numel(specified_blocks)
    ...
    if not(any(logical_index))
      ...
      msg = CodeUtil1.i18n("Specified block was found: " + specified_blocks(idx));
...
for idx = 1 : num_blocks
  ...
  if ModelUtil1.isSimulink1DLookupTableBlock(block_path)
    xdata = eval(get_param(block_path, "BreakpointsForDimension1"));
    xmaxvalues(idx) = xdata(end);
  elseif ModelUtil1.isSimscapePSLookupTable1DBlock(block_path)
...
for idx = 1 : num_blocks
  ...
  if ModelUtil1.isSimulink1DLookupTableBlock(block_fullpath)
    ...
    SignalUtil1.plotSimulink1DLookupTableBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
  ...
  elseif ModelUtil1.isSimscapePSLookupTable1DBlock(block_fullpath)
    unit_text = string(get_param(block_fullpath, "f_unit"));
    if unit_text ~= "1"
      title_text = title_text + " (" + unit_text + ")";
    end  % if
    SignalUtil1.plotSimscapePSLookupTable1DBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
      DivisionType = NameValuePair.DivisionType, ...
      Divisions = NameValuePair.Divisions, ....
      InterpolationInterval = NameValuePair.InterpolationInterval, ...
      Title = title_text )
