...
% Editable parameters
physval_length = mus1.CodeUtil.PhysicalValue(UnitText="m");  % !test-target

% Derived (read-only) parameters
physval_area = mus1.CodeUtil.PhysicalValue(UnitText="m^2");  % !test-target
...
%%
arguments (Input)
  Length (1,1) mus1.CodeUtil.PhysicalValue
  Area (1,1) mus1.CodeUtil.PhysicalValue
end  % arguments
...
%%
  ...
  function react_LengthUI_ValueChanged()
    ...
    length_info_ui.Value = mus1.CodeUtil.stringify(value(sscval_length)) + " (" + string(unit(sscval_length)) + ")";

    sscval_area = physval_area.SimscapeValue;
    area_ui.Value = mus1.CodeUtil.stringify(value(sscval_area)) + " (" + string(unit(sscval_area)) + ")";
  end  % nested function
...
length_info_ui = mus1.AppUtil.Component.EditField(row_grid);
