classdef uiUptodateTest_CodeUtil < matlab.uitest.TestCase
  ...
  methods (Test)
    ...
    function app_screenshot_dark_1(testcase)
      ...
      % R2025a or newer
      source = mus1.FileUtil.getFileFullPath("demoapp_PhysicalValue_1");
      destination = fullfile(pwd, "screenshot-DemoApp-PhysicalValue-1-dark.png");

      if not(isfile(destination)) || mus1.FileUtil.sourceFileIsNewer(Source=source, Destination=destination)
        app = demoapp_PhysicalValue_1;  % !screenshot-target
    ...
    function app_screenshot_light_1(testcase)
      ...
      if isMATLABReleaseOlderThan("R2025a")
        % R2024b
        destination = fullfile(pwd, "screenshot-DemoApp-PhysicalValue-1-24b.png");
      else
        % R2025a or newer
        destination = fullfile(pwd, "screenshot-DemoApp-PhysicalValue-1-light.png");
      end  % if
    ...
    function app_screenshot_dark_2(testcase)
      ...
      % R2025a or newer
      source = mus1.FileUtil.getFileFullPath("demoapp_PhysicalValue_2_derive");
      destination = fullfile(pwd, "screenshot-DemoApp-PhysicalValue-2-dark.png");

      if not(isfile(destination)) || mus1.FileUtil.sourceFileIsNewer(Source=source, Destination=destination)
        app = demoapp_PhysicalValue_2_derive;  % !screenshot-target
        app.Window.MainFigure.Theme = "dark";
        drawnow
        exportapp(app.Window.MainFigure, destination)
      else
    ...
    function app_screenshot_light_2(testcase)
      ...
      if isMATLABReleaseOlderThan("R2025a")
        % R2024b
        destination = fullfile(pwd, "screenshot-DemoApp-PhysicalValue-2-24b.png");
      else
        % R2025a or newer
        destination = fullfile(pwd, "screenshot-DemoApp-PhysicalValue-2-light.png");
      end  % if
