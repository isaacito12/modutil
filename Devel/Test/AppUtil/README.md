# AppUtil API version 7

Version 7 for MATLAB R2025a or newer

August 2025

![AppUtil Logo](+mus1.AppUtil/AppUtil-icon-150x150.png)

This is **AppUtil API**, MATLAB application programming interface (API)
to build apps programatically using [`uifigure`][url-uifig] and
[`uigridlayout`][url-uigrid].
AppUtil API provides a thin layer between MATLAB UI functions and
...
To use AppUtil API, your MATLAB paths must include a folder
which contains the `+mus1.AppUtil` folder.

The main window in AppUtil is `uifigure`.
Components in AppUtil are placed in `uigridlayout` using
AppUtil's component placement logic
which makes it quick and easy to build
apps with consistent look and feel.

AppUtil's highlights
