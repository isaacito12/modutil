classdef unittest_searchText < matlab.unittest.TestCase
  ...
  methods (Test)
    ...
    function PassingTest_1(~)
      mus1.SearchUtil.searchText("Copyright", FileTypes="*.m");
    end  % function

    function Error_1(testcase)
      verifyError(testcase, @test_target, "MATLAB:minrhs")
      function test_target()
        mus1.SearchUtil.searchText;  % !test-target
      end  % function
    end  % function

    function Error_2(testcase)
      verifyError(testcase, @test_target, "TextSearcher:InvalidFileTypes")
      function test_target()
        mus1.SearchUtil.searchText("")  % !test-target
      end  % function
    end  % function
    ...
    function Error_3(testcase)
      verifyError(testcase, @test_target, "TextSearcher:InvalidFileTypes")
      function test_target()
        mus1.SearchUtil.searchText("", FileType="")  % !test-target
      end  % function
    end  % function
    ...
    function Error_4(testcase)
      verifyError(testcase, @test_target, "TextSearcher:InvalidFileTypes")
      function test_target()
        mus1.SearchUtil.searchText("", FileType=["*.m", ""])  % !test-target
      end  % function
    end  % function
    ...
    function Test_1(testcase)
      % No matching files.
      session = mus1.SearchUtil.searchText("", FileType="*.test_extension");
      verifyTrue(testcase, isempty(session.Result))
    end  % function
    ...
    function Test_2(testcase)
      ...
      result = mus1.SearchUtil.searchText("Copyright", FileType = "*.m");
    ...
    function Test_3(testcase)
      ...
      verifyEqual(testcase, height(session.Result), 1)
      verifyTrue(testcase, contains(session.Result.LineText(1), "search result for testing searchText"))
    end  % function
    ...
    function Test_4(testcase)
      ...
      verifyTrue(testcase, height(session.Result) > 1)
    end  % function
    ...
    function Test_5(testcase)
      ...
      verifyTrue(testcase, any(endsWith(session.Result.FilePath, ".m")))
      verifyTrue(testcase, any(endsWith(session.Result.FilePath, ".md")))
      verifyTrue(testcase, any(endsWith(session.Result.FilePath, ".mdl")))
      verifyTrue(testcase, all(contains(session.Result.LineText, "Copyright", IgnoreCase=true)))
    end  % function
