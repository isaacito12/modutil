classdef BlockSelectorUI < AppUtil1.Component.ComponentBase
  ...
  properties
    ...
    %
    % AppUtil1.Utility.find1DLookupTableBlocks function can be used for FindBlockCallback.
    % It adds Simulink 1-D Lookup Table blocks in the Block Path drop down.
    FindBlockCallback {CodeUtil1.mustBeFunctionHandleOrEmpty}
    ...
    SetParametersToBlockCallback (:,1) {CodeUtil1.mustBeFunctionHandleOrEmpty}

    GetParametersFromBlockCallback (:,1) {CodeUtil1.mustBeFunctionHandleOrEmpty}
    ...
    GetParametersFromBlockUI AppUtil1.Component.Button
  ...
  methods
    ...
    function set.ModelFileFullPath(component, ModelFilePath)
      ...
      if not(isfile(ModelFilePath))
        ...
        msg = CodeUtil1.i18n("Invalid model file: ") + ModelFilePath;
    ...
    function set.BlockPath(component, block_path)
      ...
      if not(any(component.BlockPathDropDownUI.Items == styled_block_path))
        ...
        msg = CodeUtil1.i18n("Specified block path is not in the block path drop down items: ") + block_path;
    ...
    function callback_change_modelfile_dropdown(component)
      ...
      if component.TargetSimscapeBlockNames == ""
        ...
        try
          result_table = ModelUtil1.findSimscapeBlocks(component.ModelName);
        catch exception
          uialert(component.MainFigure, exception.message, CodeUtil1.i18n("Error"))
      ...
      if not(isempty(component.FindBlockCallback)) && isa(component.FindBlockCallback, 'function_handle')
        ...
        if not(iscolumn(block_paths_from_callback) && class(block_paths_from_callback) == "string")
          msg = CodeUtil1.i18n("FindBlockCallback must return a scalar or a column vector containing block paths as string.");
          if component.MainFigure.Visible
            title_word = CodeUtil1.i18n("Error");
            uialert(component.MainFigure, msg, title_word)
      ...
      if num_blocks == 0
        msg = CodeUtil1.i18n("Block was not found: ") + component.TargetSimscapeBlockNames;
        if component.MainFigure.Visible
          title_word = CodeUtil1.i18n("Error");
          uialert(component.MainFigure, msg, title_word)
    ...
    function subcallback_make_block_path_ready(component)
      ...
      if component.BlockPathDropDownUI.Value == ""
        ...
        msg = CodeUtil1.i18n("BlockPath must be specified.");
    ...
    function callback_open_model(component, NameValuePair)
      ...
      if NameValuePair.ModelFileFullPath ~= ""
        ...
        if not(isfile(component.ModelFileFullPath))
          ...
          msg = CodeUtil1.i18n("Invalid file was specified: ") + NameValuePair.ModelFileFullPath;
      ...
      if component.Reporting
        FileUtil1.displayTimeAndFileLocation("ModelFileDropDownUI.Value: " + component.ModelFileDropDownUI.Value)
      end  % if
    end  % function
    ...
    function callback_change_blockpath_dropdown(component)
      %%
      if component.Reporting
        FileUtil1.displayTimeAndFileLocation("Start")
      end  % if

      if component.ModelFileDropDownUI.Value == ""
        if component.Reporting
          FileUtil1.displayTimeAndFileLocation("Selected empty model file.")
        end  % if
      ...
      if getSimulinkBlockHandle(block_path) <= 0
        msg = CodeUtil1.i18n("Block was not found: " + block_path);
        title_word = CodeUtil1.i18n("Error");
        uialert(component.MainFigure, msg, title_word)
    ...
    function callback_hilit(component)
      ...
      if component.ModelFileDropDownUI.Value == ""
        ...
        msg = CodeUtil1.i18n("To highlight a block, model file must be non-empty.");
      ...
      if selected_block_display_path == ""
        id = component.errorID + "InvalidBlockPath";
        msg = CodeUtil1.i18n("To highlight a block, block path must be non-empty.");
      ...
      if component.HilitBlockUI.Value
        ...
        if getSimulinkBlockHandle(block_path) <= 0
          ...
          msg = CodeUtil1.i18n("Specified block is invalid: " + block_path);
