classdef uitest_EnabledButton < matlab.uitest.TestCase
  ...
  methods (Test)
    ...
    function app_launches_without_warnings_1(testcase)
      ...
      function target()
        testcase.App = apptest_EnabledButton_1_simplest;  % !test-target
      end  % nested function
    end  % function
    ...
    function app_launches_without_warnings_2(testcase)
      verifyWarningFree(testcase, @() target())
      function target()
        testcase.App = apptest_EnabledButton_2_fit;  % !test-target
      end  % nested function
    end  % function
    ...
    function app_launches_without_warnings_3(testcase)
      verifyWarningFree(testcase, @() target())
      function target()
        testcase.App = apptest_EnabledButton_3_horizontal_alignment;  % !test-target
      end  % nested function
    end  % function
    ...
    function Gesture_1(testcase)
      ...
      % The button must work if it is enabled.
      press(testcase, testcase.App.EnabledButtonUI.ButtonUI.MainButton)

      % The state of the check box after pressing twice must be the same as the initial state.
      v1 = testcase.App.EnabledButtonUI.CheckBoxUI.Value;
      press(testcase, testcase.App.EnabledButtonUI.CheckBoxUI.MainCheckBox)
      press(testcase, testcase.App.EnabledButtonUI.CheckBoxUI.MainCheckBox)
      v2 = testcase.App.EnabledButtonUI.CheckBoxUI.Value;
