classdef uitest_PolarAxes < matlab.uitest.TestCase
  ...
  methods (Test)
    ...
    function app_launches_without_warnings_1(testcase)
      ...
      function target()
        testcase.App = apptest_PolarAxes_1_simplest;  % !test-target
      end  % nested function
    end  % function
    ...
    function app_launches_without_warnings_2(testcase)
      verifyWarningFree(testcase, @() target())
      function target()
        testcase.App = apptest_PolarAxes_2_polarplot;  % !test-target
      end  % nested function
    end  % function
