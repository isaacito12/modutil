classdef uitest_Label < matlab.uitest.TestCase
  ...
  methods (Test)
    ...
    function app_launches_without_warnings_1(testcase)
      ...
      function target()
        testcase.App = apptest_Label_1_simplest;  % !test-target
      end  % nested function
    end  % function
    ...
    function app_launches_without_warnings_2(testcase)
      verifyWarningFree(testcase, @() target())
      function target()
        testcase.App = apptest_Label_2_inner_align;  % !test-target
      end  % nested function
    end  % function
    ...
    function app_launches_without_warnings_3(testcase)
      verifyWarningFree(testcase, @() target())
      function target()
        testcase.App = apptest_Label_3_wordwrap;  % !test-target
      end  % nested function
    end  % function
    ...
    function LightTheme_1(testcase)
      testcase.App = apptest_Label_1_simplest;
      drawnow
    ...
    function LightTheme_2(testcase)
      testcase.App = apptest_Label_2_inner_align;
      drawnow
      if not(isMATLABReleaseOlderThan("R2025a"))
        testcase.App.MainFigure.Theme = "light";
      end  % if
