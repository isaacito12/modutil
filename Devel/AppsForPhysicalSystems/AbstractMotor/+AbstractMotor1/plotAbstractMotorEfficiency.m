...
arguments (Input)
  ...
  NameValuePair.MaxSpeed (1,1) simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.MaxSpeed, "rad/s"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(17000, "rpm")

  NameValuePair.MaxTorque (1,1) simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.MaxTorque, "N*m"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(160, "N*m")

  NameValuePair.MaxPower (1,1) simscape.Value ...
      { simscape.mustBeCommensurateUnit(NameValuePair.MaxPower, "kW"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(55, "kW")

  NameValuePair.MeasuredEfficiencyPercent (1,1) double ...
    { mustBeInRange(NameValuePair.MeasuredEfficiencyPercent, 0, 100) } = 95

  NameValuePair.MeasuredAngularSpeed (1,1) simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.MeasuredAngularSpeed, "rpm"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(2000, "rpm")

  NameValuePair.MeasuredTorque (1,1) simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.MeasuredTorque, "N*m"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(50, "N*m")

  NameValuePair.MeasuredIronLoss (1,1) simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.MeasuredIronLoss, "W"), CodeUtil1.mustBeSimscapeValueNonnegative } ...
    = simscape.Value(55, "W")

  NameValuePair.FixedLoss (1,1) simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.FixedLoss, "W"), CodeUtil1.mustBeSimscapeValueNonnegative } ...
    = simscape.Value(40, "W")

  NameValuePair.RotorDamping (1,1) simscape.Value ...
      { simscape.mustBeCommensurateUnit(NameValuePair.RotorDamping, "N*m/rpm"), CodeUtil1.mustBeSimscapeValueNonnegative } ...
  ...
  NameValuePair.ContourLevelsPercent (1,:) double ...
    ...
    = [1 60 80 90 92 94 96 97 98 99]

  NameValuePair.PlotAngularSpeedMax simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.PlotAngularSpeedMax, "rad/s"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(18000, "rpm")
    % simscape.Value(1885, "rad/s")

  NameValuePair.PlotTorqueMax simscape.Value ...
    { simscape.mustBeCommensurateUnit(NameValuePair.PlotTorqueMax, "N*m"), CodeUtil1.mustBeSimscapeValuePositive } ...
    = simscape.Value(200, "N*m")
    % simscape.Value(147.5, "lbf*ft")
...
if NameValuePair.DataSource == "direct"
  ...
  plot_speed_unit = string(unit(NameValuePair.PlotAngularSpeedMax));
  plot_speed_upper_bound = value(NameValuePair.PlotAngularSpeedMax);

  plot_torque_unit = string(unit(NameValuePair.PlotTorqueMax));
  plot_torque_upper_bound = value(NameValuePair.PlotTorqueMax);

  max_motor_speed_radps = value(NameValuePair.MaxSpeed, "rad/s");
  max_motor_speed_in_plot_unit = value(NameValuePair.MaxSpeed, plot_speed_unit);
  % !attention: Speed should be non zero because it is used in the denominator when calculating torque envelope.
  ...
  max_motor_torque_in_plot_unit = value(NameValuePair.MaxTorque, plot_torque_unit);
  ...
  valid_torque_region_mask = trq_Nm_mat < torque_envelope;
  ...
  % External data set.
  % Some parameters such as MaxSpeed are the fields of NameValuePair.DataSet.
  ...
  plot_speed_unit = string(unit(DataSet.PlotAngularSpeedMax));
  plot_speed_upper_bound = value(DataSet.PlotAngularSpeedMax);

  plot_torque_unit = string(unit(DataSet.PlotTorqueMax));
  plot_torque_upper_bound = value(DataSet.PlotTorqueMax);
...
contourf(ax, speed_vector_in_plot_unit, torque_vector_in_plot_unit, efficiency_percent_mat, contour_levels, ShowText="on")
