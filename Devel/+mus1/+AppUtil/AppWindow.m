classdef AppWindow < handle
  ...
  properties
    ...
    MainVerticalContainer mus1.AppUtil.VerticalContainer

    HeaderUI mus1.AppUtil.Component.WindowHeader
  ...
  properties (Constant, Access=private)
    ...
    % The default icon file must exist in the "+mus1.AppUtil" namespace folder.
  ...
  methods
    ...
    function AppWindow = AppWindow(MainFigure, NameValuePair)
      ...
      if AppWindow.Reporting
        mus1.FileUtil.displayTimeAndFileLocation("Constructor")
      end  % if

      AppWindow.MainFigure = MainFigure;

      AppWindow.MainVerticalContainer = mus1.AppUtil.VerticalContainer(MainFigure);

      AppWindow.Icon = AppWindow.DefaultIcon;

      v_layout = addVerticalGridLayout(AppWindow.MainVerticalContainer);
      AppWindow.HeaderUI = mus1.AppUtil.Component.WindowHeader(v_layout);
    ...
    function set.Icon(AppWindow, PNGFilename)
      ...
      if PNGFilename == AppWindow.DefaultIcon
        try
          AppUtil_folder_fullpath = mus1.FileUtil.getFolderFullPath(fullfile("+mus1", "+AppUtil"));
        catch exception
        ...
        % Custom icon
        try
          icon_fullpath = mus1.FileUtil.getFileFullPath(PNGFilename);
        catch exception
          if AppWindow.MainFigure.Visible
            title_word = mus1.CodeUtil.i18n("Error");
