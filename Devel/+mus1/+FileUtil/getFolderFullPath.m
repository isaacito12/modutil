...
if FolderName == ""
  ...
  msg = mus1.CodeUtil.i18n("Empty folder name is not allowed.");
...
%}
  ...
  if not(endsWith(found_fullpaths, FolderName))
    ...
    %   msg = mus1.CodeUtil.i18n("Folder was not found: ") + FolderName;
    % 
    %   throw(MException(id, msg))
    % 
    % end  % if
...
if ischar(found_fullpaths) || (isstring(found_fullpaths) && isscalar(found_fullpaths))
  ...
  if NameValuePair.WarningOnMultipleMatch
    ...
    msg = mus1.CodeUtil.i18n("There are two or more matches: ") + numel(found_fullpaths);
