...
if not(issorted(x_points(:,1), "strictascend"))
  ...
  Result.Message = mus1.CodeUtil.i18n("X start vector must be strictly asending.");
...
if any(logical_index)
  ...
  if not(issorted(tmp_vec, "strictascend"))
    ...
    Result.Message = mus1.CodeUtil.i18n("X end vector must be strictly asending.");
  ...
  if not(isempty(logical_index))
    ...
    Result.Message = mus1.CodeUtil.i18n("X start is after X end, which is invalid, at rows: ") + num2str(logical_index');
...
if not(all(not(isnan(f_points))))
  ...
  Result.Message = mus1.CodeUtil.i18n("F data cannot have NaN.");
