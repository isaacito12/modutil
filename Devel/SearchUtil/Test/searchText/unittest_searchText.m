classdef unittest_searchText < matlab.unittest.TestCase
  ...
  methods (Test)
    ...
    function PassingTest_1(~)
      SearchUtil1.searchText("Copyright", FileTypes="*.m");
    end  % function

    function Error_1(testcase)
      verifyError(testcase, @test_target, "MATLAB:minrhs")
      function test_target()
        SearchUtil1.searchText;  % !test-target
      end  % function
    end  % function
    ...
    function Error_3(testcase)
      verifyError(testcase, @test_target, "TextSearcher:InvalidFileTypes")
      function test_target()
        SearchUtil1.searchText("", FileType="")  % !test-target
      end  % function
    end  % function
    ...
    function Error_4(testcase)
      verifyError(testcase, @test_target, "TextSearcher:InvalidFileTypes")
      function test_target()
        SearchUtil1.searchText("", FileType=["*.m", ""])  % !test-target
      end  % function
    end  % function
    ...
    function Test_1(testcase)
      % No matching files.
      session = SearchUtil1.searchText("", FileType="*.test_extension");
