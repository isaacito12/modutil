classdef TextSearcher < handle
  ...
  properties
    States (1,1) SearchUtil1.TextSearchStates = SearchUtil1.TextSearchStates
  ...
  methods
    ...
    function App = TextSearcher(NameValuePair)
      ...
      arguments (Input)
        ...
        NameValuePair.States (1,:) SearchUtil1.TextSearchStates
      end  % arguments
    ...
    function setStates(searcher, states)
      ...
      arguments (Input)
        searcher
        states (1,1) SearchUtil1.TextSearchStates
      end  % arguments
    ...
    function buildFileTypes(searcher)
      ...
      if isempty(file_types) || (isscalar(file_types) && file_types == "")
        ...
        msg = CodeUtil1.i18n("File types must be specified.");
    ...
    function argumentText = getCommandArgumentText(searcher)
      ...
      if not(ready(searcher))
        ...
        msg = CodeUtil1.i18n("Search is not ready. Specify all required options.");
      ...
      assert(k < upper_bound, searcher.errorID+"FatalError", CodeUtil1.i18n("Fatal error"))
    ...
    function result = runSearch(searcher)
      ...
      if searcher.States.ExcludeLiveScript
        collection = select(collection, @(x) not(FileUtil1.isPlainTextLiveScript(x)));
      end  %if

      if searcher.States.ExcludeMATLABCodeFile
        collection = select(collection, @(x) endsWith(x, ".m") & FileUtil1.isPlainTextLiveScript(x));
      end  %if
