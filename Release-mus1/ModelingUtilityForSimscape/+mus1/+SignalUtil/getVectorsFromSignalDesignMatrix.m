...
check_result = mus1.SignalUtil.checkSignalDesignMatrix(SignalDesignMatrix);
...
assert( issorted(x_points(:,1), "strictascend"), ...
  ...
  mus1.CodeUtil.i18n("X start vector must be strictly asending."))
...
if any(logical_index)
  ...
  assert( issorted(tmp_vec, "strictascend"), ...
    ...
    mus1.CodeUtil.i18n("X end vector must be strictly asending."))
  ...
  assert( isempty(logical_index), ...
    errorID + "InvalidX", ...
    mus1.CodeUtil.i18n("X start is after X end, which is invalid, at these rows: ") + num2str(logical_index'))
...
assert( all(not(isnan(f_points))), ...
  errorID + "invalidF", ...
  mus1.CodeUtil.i18n("F data cannot have NaN."))
%}
...
assert( all(dx > 0), ...
  ...
  mus1.CodeUtil.i18n("X data points are not strictly ascending."))
