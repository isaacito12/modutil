classdef TextSearcher < handle
  ...
  % the mus1.SearchUtil.searchText command or the TextSearchApp.
  %
  ...
  properties
    States (1,1) mus1.SearchUtil.TextSearchStates = mus1.SearchUtil.TextSearchStates
  ...
  methods
    ...
    function App = TextSearcher(NameValuePair)
      ...
      arguments (Input)
        ...
        NameValuePair.States (1,:) mus1.SearchUtil.TextSearchStates
      end  % arguments
    ...
    function setStates(searcher, states)
      ...
      arguments (Input)
        searcher
        states (1,1) mus1.SearchUtil.TextSearchStates
      end  % arguments
    ...
    function buildFileTypes(searcher)
      ...
      if isempty(file_types) || (isscalar(file_types) && file_types == "")
        ...
        msg = mus1.CodeUtil.i18n("File types must be specified.");
    ...
    function argumentText = getCommandArgumentText(searcher)
      ...
      if not(ready(searcher))
        ...
        msg = mus1.CodeUtil.i18n("Search is not ready. Specify all required options.");
      ...
      assert(k < upper_bound, searcher.errorID+"FatalError", mus1.CodeUtil.i18n("Fatal error"))
    ...
    function result = runSearch(searcher)
      ...
      if searcher.States.ExcludeLiveScript
        collection = select(collection, @(x) not(mus1.FileUtil.isPlainTextLiveScript(x)));
      end  %if

      if searcher.States.ExcludeMATLABCodeFile
        collection = select(collection, @(x) endsWith(x, ".m") & mus1.FileUtil.isPlainTextLiveScript(x));
      end  %if
