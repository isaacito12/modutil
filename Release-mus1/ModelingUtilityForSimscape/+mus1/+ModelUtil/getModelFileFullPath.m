...
if ModelName == ""
  ...
  msg = mus1.CodeUtil.i18n("Model name must be non-empty.");
...
result = mus1.FileUtil.getFileFullPath(ModelName, ReturnIfNotFound=true);
if result ~= ""
  if not(endsWith(result, ".mdl")) && not(endsWith(result, ".slx"))
    id = errorID + "InvalidFileName";
    msg = mus1.CodeUtil.i18n("Model file must ends with "".mdl"" or "".slx"".");
...
result = mus1.FileUtil.getFileFullPath(ModelName+".mdl", ReturnIfNotFound=true);
if result ~= ""
  FullPath = result;
...
result = mus1.FileUtil.getFileFullPath(ModelName+".slx", ReturnIfNotFound=true);
if result ~= ""
  FullPath = result;
...
msg = mus1.CodeUtil.i18n("Model file was not found.");
