...
if SubsystemPath == ""
  if isempty(gcs)
    ...
    msg = mus1.CodeUtil.i18n("Model must be specified.");
...
all_blocks = mus1.ModelUtil.findLookupTable1DBlocks(SubsystemPath, SearchDepth = NameValuePair.SearchDepth);
num_blocks = numel(all_blocks);
if num_blocks == 0

  id = errorID + "BlocksWereNotFound";
  msg = mus1.CodeUtil.i18n("No lookup table blocks were found.");
...
if not(isfield(NameValuePair, "Blocks"))
  ...
  for idx = 1 : numel(specified_blocks)
    ...
    if not(any(logical_index))
      ...
      msg = mus1.CodeUtil.i18n("Specified block was found: " + specified_blocks(idx));
...
for idx = 1 : num_blocks
  ...
  if mus1.ModelUtil.isSimulink1DLookupTableBlock(block_fullpath)
    ...
    mus1.SignalUtil.plotSimulink1DLookupTableBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
  ...
  elseif mus1.ModelUtil.isSimscapePSLookupTable1DBlock(block_fullpath)
    unit_text = string(get_param(block_fullpath, "f_unit"));
    if unit_text ~= "1"
      title_text = title_text + " (" + unit_text + ")";
    end  % if
    mus1.SignalUtil.plotSimscapePSLookupTable1DBlock(block_fullpath, ParentAxes=nexttile(vertical_tile), PlotXUpperBound=x_max, ...
      DivisionType = NameValuePair.DivisionType, ...
      Divisions = NameValuePair.Divisions, ....
      InterpolationInterval = NameValuePair.InterpolationInterval, ...
      Title = title_text )
