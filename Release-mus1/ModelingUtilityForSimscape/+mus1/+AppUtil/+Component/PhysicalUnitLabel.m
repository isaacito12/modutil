classdef PhysicalUnitLabel < mus1.AppUtil.Component.ComponentBase
  ...
  properties
    ...
    LabelUI mus1.AppUtil.Component.Label
  ...
  methods (Access=protected)
    ...
    function setup(component)
      ...
      component.LabelUI = mus1.AppUtil.Component.Label(component.main_grid);
      ...
      component.ComponentHeight = mus1.AppUtil.Constant.Height{"oneline++"};
  ...
  methods
    ...
    function set.UnitText(component, NewUnitText)
      ...
      if component.UnitSpecified
        if component.AliasSpecified
          ...
          if component.Reporting
            mus1.FileUtil.displayTimeAndFileLocation("2:alias")
          end  % if
          if NewUnitText ~= "1"
            id = component.errorID + "InvalidUnitForAlias";
            msg = mus1.CodeUtil.i18n("Unit alias is defined. ""1"" is the only allowed unit: " + NewUnitText);
          ...
          if component.Reporting
            mus1.FileUtil.displayTimeAndFileLocation("3:modify:" + NewUnitText)
          end  % if
          if not(simscape.isCommensurateUnit(NewUnitText, component.CurrentUnitText))
            id = component.errorID + "UnitMustBeCommensurate";
            msg = mus1.CodeUtil.i18n("New unit must be commensurate with the current unit.");
        ...
        if component.Reporting
          mus1.FileUtil.displayTimeAndFileLocation("4:new:" + NewUnitText)
        end  % if
        try
    ...
    function set.UnitAlias(component, NewAliasString)
      ...
      if not(component.UnitSpecified)
        if component.Reporting
          mus1.FileUtil.displayTimeAndFileLocation("2:specify:" + NewAliasString)
        end  % if
      ...
      if component.CurrentUnitText ~= "1"
        if component.Reporting
          mus1.FileUtil.displayTimeAndFileLocation("3:error")
        end  % if
        id = component.errorID + "CannotSpecifyAlias";
        msg = mus1.CodeUtil.i18n("Specifying alias is supported only if unit is ""1"".");

        throw(MException(id, msg))

      end  % if
      if component.Reporting
        mus1.FileUtil.displayTimeAndFileLocation("4:modify:" + NewAliasString)
      end  % if
      component.LabelUI.Text = NewAliasString;
      component.AliasSpecified = true;
      notify(component, "UnitLabelChanged")
